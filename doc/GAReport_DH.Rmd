...
library(googleVis)
t1 <- gvisTable(gadata[1:1000,],options = list(width = 800, height = min(400,27*(nrow(gadata) + 1)), allowHTML = TRUE, page = 'disable'))
print(t1,'chart')
```
...
#adding avg_time_on_page
gadata[,"avg_time_on_page"]=gadata[,which(colnames(gadata)=="timeOnPage")]/(gadata[,which(colnames(gadata)=="pageviews")]-gadata[,which(colnames(gadata)=="exits")])

#adding bounce_rate
gadata[,"bounce_rate"]=gadata[,which(colnames(gadata)=="exits")]/gadata[,which(colnames(gadata)=="entrances")]

#adding exit_rate
gadata[,"exit_rate"] = gadata[,which(colnames(gadata)=="exits")]/gadata[,which(colnames(gadata)=="pageviews")]
```
